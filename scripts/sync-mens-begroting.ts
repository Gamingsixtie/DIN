// Pas mens (gespreksvaardigheid) totaalEuro per scenario aan zodat
// berekening (uit known-breakdowns) sluit op begroting in §4.1.
//
// Doelwaarden:
//   optimaal (7j): €200.000 → €222.500  (+€22.500)
//   plus20   (5j): €170.000 → €182.500  (+€12.500)
//   advies   (4j): €155.000 → €162.500  (+€7.500)
//   min20  (10j): €230.000 → €282.500  (+€52.500)
//
// Onderbouwing: €142.500 eenmalig (LMS+content+ToT+nulmeting+kerntraject+
// sessieondersteuning) + €20K/jr (refresh €15K + onboarding €5K) × actieve
// jaren vanaf jaar 4.
//
...
async function main() {
  ...
  for (const [sk, target] of Object.entries(TARGETS)) {
    ...
    const insp = (sc.inspanningen ?? []).find((i) => /gespreksvaardigh/i.test(i.inspanningTitel as string));
    if (!insp) { console.log(`(${sk}) gespreksvaardigheid niet gevonden — overslaan`); continue; }

    const huidig = (insp.totaalEuro as number) ?? 0;
    const delta = target - huidig;
    console.log(`(${sk}) gespreksvaardigheid: ${fmt(huidig)} → ${fmt(target)}  (delta ${delta >= 0 ? "+" : ""}${fmt(delta)})`);
    if (delta === 0) continue;
      ...
      if (totaalOud > 0) {
        ...
        for (let i = 0; i < verd.length; i++) {
          if (i === verd.length - 1) {
            ...
            const nieuw = Math.round(((verd[i].euro ?? 0) * factor) / 500) * 500;
            verd[i].euro = nieuw;
            lopend += nieuw;
          }
        }
