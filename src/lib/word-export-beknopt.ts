...
/** Eén kaartje: accentbalk links + inhoudscel. */
export function kaartje(o: KaartjeOpts): Table {
  ...
  for (const alinea of o.body ?? []) {
    ...
    inhoud.push(
      new Paragraph({
        spacing: { after: 60, line: 280 },
        children: [new TextRun({ text: alinea, size: 19, color: tekstKleur, font: "Calibri" })],
      })
    );
  }
    ...
    if (s.tekst) {
      inhoud.push(
        new Paragraph({
          ...
          children: [new TextRun({ text: s.tekst, size: 18, color: tekstKleur, font: "Calibri" })],
        })
      );
    }
    for (const b of s.bullets ?? []) {
      inhoud.push(
        new Paragraph({
          ...
          indent: { left: 180 },
          children: [
            new TextRun({ text: "•  ", color: o.accent, font: "Calibri", size: 18 }),
            new TextRun({ text: b, size: 18, color: tekstKleur, font: "Calibri" }),
          ],
        })
      );
    }
    ...
    for (const [k, v] of o.profiel.rijen) {
      ...
      inhoud.push(
        new Paragraph({
          spacing: { after: 20, line: 260 },
          children: [
            new TextRun({ text: `${k}: `, bold: true, size: 17, color: TEXT_SECONDARY, font: "Calibri" }),
            new TextRun({
              text: v,
              size: 17,
...
/**
 * Zet kaartjes naast elkaar in een raster. De kaartjes worden zelf zonder
...
export function kaartjesRaster(kaarten: Table[], kolommen: number, totaleBreedte: number): Table[] {
  ...
  for (let i = 0; i < kaarten.length; i += kolommen) {
    const groep = kaarten.slice(i, i + kolommen);
    const cellen = groep.map(
      (k) =>
        new TableCell({
    ...
    // Rij aanvullen tot volledige breedte, anders rekt Word de laatste cel op.
    while (cellen.length < kolommen) {
      cellen.push(
        new TableCell({
          width: { size: kolBreedte, type: WidthType.DXA },
          borders: { top: NIL_BORDER, bottom: NIL_BORDER, left: NIL_BORDER, right: NIL_BORDER },
    ...
    rijen.push(new TableRow({ children: cellen }));
  }

  return [
    new Table({
      width: { size: totaleBreedte, type: WidthType.DXA },
      layout: TableLayoutType.FIXED,
      columnWidths: Array(kolommen).fill(kolBreedte),
      borders: {
        top: NIL_BORDER,
        bottom: NIL_BORDER,
        left: NIL_BORDER,
...
export interface BeknoptData {
  ...
  planning: { samenvatting: string; toelichting: string; bundels: BundelBeknopt[] } | null;
...
export function kiesActiefScenario(session: DINSession): ScenarioK {
  ...
  if (begroting?.scenarios?.advies) return "advies";
  return "optimaal";
}
...
function bouwGeld(session: DINSession, open: OpenPunt[]): GeldBeknopt | null {
  ...
  if (!vastgelegd) {
    open.push({
      hoofdstuk: "4. Wat het kost",
      tekst: `Er is nog geen scenario vastgelegd in cross-analyse stap 8; dit document rekent met "${SCENARIO_LABELS[scenario]}".`,
    });
  }
...
function bouwInspanningen(session: DINSession, open: OpenPunt[]): InspanningBeknopt[] {
  ...
  perDomein.forEach((se, index) => {
    ...
    const hoofdstuk = "3. De vier inspanningen";
...
function bouwPlanning(session: DINSession, open: OpenPunt[]) {
  ...
  return {
    samenvatting: tekst(planning.samenvatting),
...
function bouwOrganisatie(session: DINSession, open: OpenPunt[]) {
  ...
  const voegToe = (rolLabel: string, rol: { rol?: string; naam?: string; functie?: string } | undefined) => {
    if (!rol) return;
    const naam = veld(rol.naam, "6. Wie", `Naam van de ${rolLabel.toLowerCase()}`, open);
...
export function verzamelBeknoptData(session: DINSession): BeknoptData {
  ...
  const focusConflict =
    focusDoelNaam && focusDoel && focusDoelNaam !== focusDoel.naam
      ...
      : "";
  if (focusConflict) open.push({ hoofdstuk: "1. Waar het programma over gaat", tekst: focusConflict });
...
function bronRegel(text: string): Paragraph {
  return bodyText(text, { italic: true, size: 16, color: TEXT_MUTED });
}
          ...
          (k) =>
            new TableCell({
              ...
              children: [
                ...
                new Paragraph({
                  spacing: { after: 0 },
                  children: [new TextRun({ text: k.sub, size: 15, color: TEXT_MUTED, font: "Calibri" })],
                }),
              ],
            })
        ),
...
function besluitenSectie(data: BeknoptData): Sectie {
  ...
  if (data.geld) {
    ...
    punten.push(
      `Raming (${data.geld.label}): ${formatEuro(data.geld.totaal)}${jaren} — ` +
        `${formatEuro(data.geld.outOfPocket)} out-of-pocket en ${formatEuro(data.geld.interneKosten)} interne uren.`
    );
  }
  ...
  if (data.planning && data.planning.bundels.length > 0) {
    ...
    if (periodes.length > 0) {
      ...
      punten.push(
        `Doorlooptijd: ${start} tot en met ${eind}, verdeeld over ${data.planning.bundels.length} bundels.`
      );
    }
  }
  ...
  if (open.length > 0) {
    punten.push(
      `${open.length} ${open.length === 1 ? "punt is" : "punten zijn"} nog niet vastgelegd — zie hoofdstuk 7.`
    );
  }
  ...
  if (data.batenPerSector.length > 0) {
    ...
    const baatKaarten = data.batenPerSector.map((s) => {
      ...
      return kaartje({
        ...
        body: [
          ...s.kaarten.slice(1).map((k) => k.titel),
          ...(eerste?.risico ? [eerste.risico] : []),
        ],
      });
    });
  ...
  const inspanningKaarten = DOMEIN_OUTSIDE_IN_ORDER.map((domein) => {
    ...
    return kaartje({
      ...
      pills: [
        ...
        insp.actie === "combineren"
          ? { tekst: "Combineren", bg: "D1FAE5", kleur: "065F46" }
...
function inspanningenSectie(data: BeknoptData, state: NumberingState): Sectie {
  ...
  children.push(
    bodyText(
      "Per domein één gezamenlijke inspanning, met het dossier zoals dat in de sessie is vastgelegd.",
      { size: 20, color: TEXT_SECONDARY }
    )
  );
  children.push(emptyLine(120));

  data.inspanningen.forEach((insp) => {
    const secties: KaartjeSectie[] = [];
    if (insp.beargumentatie) {
      secties.push({ label: "Waarom cross-sectoraal opbouwen?", tekst: insp.beargumentatie });
    }
    if (insp.vermogenImpact.length > 0) {
      secties.push({
        label: "Wat dit per sector oplevert",
        bullets: insp.vermogenImpact.map((vi) => `${vi.sector} — ${vi.impact}`),
      });
    }
    ...
    if (insp.gebundeld.length > 0) {
      meta.push(["Gebundelde sector-inspanningen", insp.gebundeld.join("; ")]);
    }
    ...
    children.push(
      kaartje({
        ...
        secties,
        profiel: {
          label: "Inspanningsdossier",
          rijen: insp.dossier.map((d) => [d.label, d.waarde] as [string, string]),
        },
        meta,
      })
    );
...
function planningSectie(data: BeknoptData, state: NumberingState): Sectie {
  ...
  if (planning.samenvatting) children.push(bodyText(planning.samenvatting, { size: 20 }));
