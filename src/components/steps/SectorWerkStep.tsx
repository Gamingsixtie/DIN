  ...
  async function handleAnalyzePlan(extraFeedback?: string) {
    ...
    try {
      ...
      if (data.success && data.data?.analysis) {
        ...
        setAnalyzeError(data.data?.message || "Analyse niet beschikbaar.");
      }
    } catch {
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg">
              <p className="text-sm text-gray-700 leading-relaxed">{currentAnalysis.samenvatting}</p>
            </div>

            {/* Aansluiting op KiB-doelen */}
            {currentAnalysis.aansluiting && (
              <div className="p-4 rounded-lg border-l-4 border-green-500 bg-green-50/50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h5 className="text-sm font-bold text-gray-800">{currentAnalysis.aansluiting.titel}</h5>
                </div>
                <p className="text-xs text-gray-500 italic mb-2">{currentAnalysis.aansluiting.toelichting}</p>
                <ul className="space-y-1.5">
                  {currentAnalysis.aansluiting.punten.map((punt, i) => (
            ...
            {currentAnalysis.baten && (
              <div className="p-4 rounded-lg border-l-4 border-blue-500 bg-blue-50/50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h5 className="text-sm font-bold text-gray-800">{currentAnalysis.baten.titel}</h5>
                </div>
                <p className="text-xs text-gray-500 italic mb-2">{currentAnalysis.baten.toelichting}</p>
                <ul className="space-y-1.5">
                  {currentAnalysis.baten.punten.map((punt, i) => (
                    <li key={i} className="text-sm text-gray-700 flex items-start gap-2">
            ...
            {currentAnalysis.vermogens && (
              <div className="p-4 rounded-lg border-l-4 border-purple-500 bg-purple-50/50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h5 className="text-sm font-bold text-gray-800">{currentAnalysis.vermogens.titel}</h5>
                </div>
                <p className="text-xs text-gray-500 italic mb-2">{currentAnalysis.vermogens.toelichting}</p>
                <ul className="space-y-1.5">
                  {currentAnalysis.vermogens.punten.map((punt, i) => (
                    <li key={i} className="text-sm text-gray-700 flex items-start gap-2">
            ...
            {currentAnalysis.inspanningen && (
              <div className="p-4 rounded-lg border-l-4 border-amber-500 bg-amber-50/30">
                <div className="flex items-center gap-2 mb-1">
                  <svg className="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                  <h5 className="text-sm font-bold text-gray-800">{currentAnalysis.inspanningen.titel}</h5>
                </div>
                <p className="text-xs text-gray-500 italic mb-3">{currentAnalysis.inspanningen.toelichting}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {(["mens", "processen", "data_systemen", "cultuur"] as const).map((domain) => {
                    const items = currentAnalysis.inspanningen[domain] || [];
            ...
            {/* Aandachtspunten */}
            {currentAnalysis.aandachtspunten && (
              <div className="p-4 rounded-lg border-l-4 border-red-400 bg-red-50/50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h5 className="text-sm font-bold text-gray-800">{currentAnalysis.aandachtspunten.titel}</h5>
                </div>
                <p className="text-xs text-gray-500 italic mb-2">{currentAnalysis.aandachtspunten.toelichting}</p>
                <ul className="space-y-1.5">
                  {currentAnalysis.aandachtspunten.punten.map((punt, i) => (
                    <li key={i} className="text-sm text-gray-700 flex items-start gap-2">
